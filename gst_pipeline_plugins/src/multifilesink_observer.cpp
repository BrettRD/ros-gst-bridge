...
namespace gst_pipeline_plugins
{
  ...
  // Attach callback to our bus
  // https://gstreamer.freedesktop.org/documentation/gstreamer/gstpipeline.html?gi-language=c#gst_pipeline_get_bus
  GstBus* bus = gst_pipeline_get_bus(GST_PIPELINE(pipeline_));
  gst_bus_add_watch(bus, &multifilesink_observer::gst_bus_cb, static_cast<gpointer>(this));
  gst_object_unref(bus);

  // bus watch messages only occur in a main loop
