  ...
  std::shared_ptr<gst_bridge::node_interface_collection> node_if, GstElement * pipeline)
{
  ...
  if (GST_IS_BIN(pipeline_)) {
    ...
    if (bin_) {
      RCLCPP_INFO(
        node_if->logging->get_logger(), "plugin gst_pipes_snapshooter '%s' found '%s'", name_.c_str(),
        elem_name_.c_str());
    ...
    RCLCPP_ERROR(
      node_if->logging->get_logger(),
      "plugin gst_pipes_snapshooter '%s' received invalid pipeline in initialisation", name_.c_str());
  }
}
...
GstPadProbeReturn
gst_pipes_snapshooter::gst_pad_probe_cb (GstPad *pad, GstPadProbeInfo *info,
  gpointer user_data)
{
