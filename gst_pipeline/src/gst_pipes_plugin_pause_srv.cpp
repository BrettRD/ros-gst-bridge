  ...
  std::shared_ptr<gst_bridge::node_interface_collection> node_if, GstElement * pipeline)
{
  ...
  seek_service_ = rclcpp::create_service<gst_msgs::srv::Seek>(
    ...
    "seek",
    std::bind(
      &gst_pipes_pause_srv::seek_srv_cb, this,
      std::placeholders::_1, std::placeholders::_2),
      rclcpp::SensorDataQoS().get_rmw_qos_profile(),
      nullptr
  );
...
void gst_pipes_pause_srv::seek_srv_cb(
  gst_msgs::srv::Seek::Request::SharedPtr req,
  gst_msgs::srv::Seek::Response::SharedPtr res)
{
  ...
  if (false == seek_return){
    RCLCPP_ERROR(node_if_->logging->get_logger(), "Failed to seek the pipeline, are we running live?");
  }
