...
class Pipeline:
  def __init__(self, node_):
    ...
    self.node = node_
    self.updater = diagnostic_updater.Updater(self.node)
    ...
    self.node.get_logger().info('startup')
  ...
  def on_status_changed(self, bus, message):
    ...
    self.node.get_logger().info(logmsg)
  ...
  def start_pipeline(self):
    self.node.get_logger().info('Current state of my pipeline is '+ Gst.Element.state_get_name(self.pipe.current_state))
    self.node.get_logger().info('setting pipeline to PLAYING')
    self.pipe.set_state(Gst.State.PLAYING)

  def add_section(self, section):
    self.node.get_logger().info('adding section ' + section.name)
    if section.bin != None:
      self.pipe.add(section.bin)
      if section.diagnostics != None:
        self.updater.add(section.diagnostics)
      else:
        self.node.get_logger().warn('section has no diagnostics')
    else:
      self.node.get_logger().warn('section is empty')
  ...
  def foreach_element_diagnostics(self, item, user_data):
    ...
    stat.add(bin_dir + " base_time", str(base_time))
